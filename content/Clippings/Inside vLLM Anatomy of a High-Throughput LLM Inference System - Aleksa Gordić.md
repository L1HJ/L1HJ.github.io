...
In this post, I'll gradually introduce all of the core system components and advanced features that make up a modern high-throughput LLM inference system. In particular I'll be doing a breakdown of how vLLM [\[1\]](https://www.aleksagordic.com/blog/#ref-1) works.
...
1. [LLM engine & engine core](https://www.aleksagordic.com/blog/#cpt1): fundamentals of vLLM (scheduling, paged attention, continuous batching, etc.)
2. [Advanced features](https://www.aleksagordic.com/blog/#cpt2): chunked prefill, prefix caching, guided & speculative decoding, disaggregated P/D
3. [Scaling up](https://www.aleksagordic.com/blog/#cpt3): from single-GPU to multi-GPU execution
4. [Serving layer](https://www.aleksagordic.com/blog/#cpt4): distributed / concurrent web scaffolding
5. [Benchmarks and auto-tuning](https://www.aleksagordic.com/blog/#cpt5): measuring latency and throughput
...
- using standard transformer [\[2\]](https://www.aleksagordic.com/blog/#ref-2) (supporting hybrid models like Jamba requires a more complex hybrid KV-cache memory allocator)
...
- Scheduler (decides which requests go into the next engine step) - it further contains:
	...
	3. KV cache manager - the heart of paged attention [\[3\]](https://www.aleksagordic.com/blog/#ref-3)
...
3. Initialize KV cache
	- Get per-layer KV-cache spec. Historically this was always `FullAttentionSpec` (homogeneous transformer), but with hybrid models (sliding window, Transformer/SSM like Jamba) it became more complex (see Jenga [\[5\]](https://www.aleksagordic.com/blog/#ref-5))
...
2. When adding a request, its status is set to `WAITING_FOR_FSM` and `grammar_init` selects the backend compiler (e.g., `xgrammar` [\[7\]](https://www.aleksagordic.com/blog/#ref-7); note that backends are 3rd party code).
...
Speculative decoding [\[8\]](https://www.aleksagordic.com/blog/#ref-8) speeds this up by introducing a smaller draft LM. The draft proposes `k` tokens cheaply. But we don't ultimately want to sample from the smaller model — it's only there to guess candidate continuations. The large model still decides what's valid.
...
vLLM V1 does not support the LLM draft model method, instead it implements faster—but less accurate—proposal schemes: n-gram, EAGLE [\[9\]](https://www.aleksagordic.com/blog/#ref-9), and Medusa [\[10\]](https://www.aleksagordic.com/blog/#ref-10).
...
Prefill and decode have very different performance profiles (compute-bound vs. memory-bandwidth-bound), so separating their execution is a sensible design. It gives tighter control over latency — both `TFTT` (time-to-first-token) and `ITL` (inter-token latency) — more on this in the [benchmarking](https://www.aleksagordic.com/blog/#cpt5) section.
